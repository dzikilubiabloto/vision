...
export const saveMinutes = async ({number, facilitator, date, short, text}, pass) => {
...
export const getAgenda = async (pass) => {
  ...
  try {
    querySnapshot.forEach((doc) => {
      ...
      documents.push({  ...doc.data(), id: doc.id, text: decryptString(doc.text, pass) });
    });
  } catch (e) {
...
export const getMinutes = async (pass) => {
    ...
    try {
      await querySnapshot.forEach(async (doc) => {
        // doc.data() is never undefined for query doc snapshots
        console.log(doc)
        documents.push({ ...doc.data(), id: doc.id, text: doc.data().text && (await decryptString(doc.data().text, pass)), facilitator: doc.data().facilitator && (await decryptString(doc.data().facilitator, pass)), short: doc.data().short && decryptString(doc.data().short, pass) });
      });
    } catch (e) {
      console.error("Error reading vision document: ", e);
    }
