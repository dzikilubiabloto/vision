...
function Vision({ vision, changeVisionField, editing, setEditing }) {
  ...
  return (
    <div className="vision-container-wrapper">
      <div>
        {currentUser && !editing && <div className="vision-edit-button" onClick={() => toggleEditing(true)}>edit</div>}
        {currentUser && editing && <div className="vision-edit-button" onClick={() => toggleEditing(false)}>stop editing and save</div>}
      </div>
      <div className="vision-container">
        ...
        <div className={"vision-markdown" + (!editing ? " editing-markdon" : "")}>
          <ReactMarkdown remarkPlugins={[gfm]} children={text}></ReactMarkdown>
        </div>
      </div>
      {currentUser && editing && <div><a href="https://demo.hedgedoc.org/features?both"> Help and instructions</a></div>}
    </div>
  );
}
